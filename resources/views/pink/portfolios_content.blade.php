        ...
        @foreach($portfolios as $portfolio)
                <div class="hentry work group">
                    <div class="work-thumbnail">
                        <div class="nozoom">
                            <img src="{{ asset(Config::get('settings.theme')) }}/images/projects/{{ $portfolio->img->max }}" alt="0061" title="0061" />
                            <div class="overlay">
            ...
            @if($portfolios->lastPage() > 1)
                ...
                @for ($i = 1; $i <= $portfolios->lastPage(); $i++ )
                    ...
                    @if($portfolios->currentPage() == $i)
                            <a class="selected disabled" href="">{{ $i }}</a>
                        @else
                            <a href="{{ $portfolios->url(1) }}">{{ $i }}</a>
                    @endif
